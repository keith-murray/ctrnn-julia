...
# ╔═╡ 2e088ddc-b6c8-4533-b65d-aca8fc9d9351
 using Lux, ComponentArrays, LinearAlgebra, SciMLSensitivity, NNlib, Optimisers, OrdinaryDiffEq, Random, Statistics, Zygote, CSV, DataFrames, DiffEqGPU, CUDA

# ╔═╡ bac6c098-5de5-4ce0-af31-b1ccc706587e
CUDA.allowscalar(false)

# ╔═╡ c5de3c6e-e541-40f1-9f2a-5454c70db86e
function loadSETdata()
	...
	accepted_len = length(accepted)
	rejected = collect(skipmissing(df."3SYYR"))
	rejected_len = length(rejected)
	return rejected, accepted
end

# ╔═╡ eeaa80df-e8e3-42b9-af5f-71086012e4ce
...
# ╔═╡ 9bc60bf1-a7ed-40bd-80b7-9a788a89f66a
begin
	...
	function (n::NeuralODE)(x, ps, st)
	    ...
	    return solve(ensemble_prob, n.solver, EnsembleGPUArray(), trajectories = length(x.funcs); sensealg=n.sensealg, n.kwargs...), st
	end
end

# ╔═╡ f859b65a-f72c-4a7b-aaea-817061b8930d
function ensemsol_to_array(x::EnsembleSolution)
	return dropdims(gpu(x); dims=2)
end

# ╔═╡ 6c743eb4-f799-4a5a-9665-17d9d04c09ad
...
# ╔═╡ aaa58b37-30f4-4c4c-8164-c72d2502f045
struct FunctionArray{F} <: AbstractArray{F, 1}
	data::Array{F, 1}
end

# ╔═╡ 8c0bb43a-3551-4a14-bdb5-5ef0b2f3412e
...
# ╔═╡ 1d17302b-637a-43ee-b143-ef563d8b1d98
struct ArrayAndFunctionArray{A <: AbstractArray, B <: FunctionArray}
	array::A
	funcs::B
end

# ╔═╡ 98f2a35a-ba36-4ee0-9d65-80126e7ebe4c
begin
...
# ╔═╡ 2f1eb592-cc38-11ed-03f1-095dfcb03225
md"# NeuralODE training on GPU"

# ╔═╡ 2162404b-62ca-4f65-90c2-e1f04601463f
md"""
The purpose of this notebook is to train a NeuralODE with SET data on a GPU. The results from this code will probably be reformated to a package.
"""

# ╔═╡ fd7b5dde-7c4d-47d9-a782-6d40f9685f2c
md"## Setup"

# ╔═╡ 8ab1df24-0f55-4315-bcd9-8399716e7d33
md"## Load SET data"

# ╔═╡ 78552d7d-e940-4669-bbc2-6690a803b878
md"## Define NeuralODE layer"

# ╔═╡ 3fd71f9a-e229-4e36-8331-6a40c0d3d58d
md"## Define input types"

# ╔═╡ 3d690db5-4286-4828-a0e2-0e95978a548f
md"## Define utility functions"

# ╔═╡ 1ed79f77-5833-4d7f-be12-b09829ddd3e5
meansquarederror(y_pred, y) =  mean(abs2, y_pred .- y)

# ╔═╡ 3a9c726d-40a8-4a4c-84e3-42533d82e482
function loss(x, y, model, ps, st)
    y_pred, st = model(x, ps, st)
    return meansquarederror(y_pred, y), st
end

# ╔═╡ 7e7872b5-a309-411f-9663-f2b75e747ad8
function test_mse(rng, model, ps, st, batch, data, IC)
	...
	mse, st = loss(cpu(x), cpu(y_expected), model, ps, st)
	return mse
end

# ╔═╡ 13e63656-ef0f-4f37-b228-59b24d927568
md"## Define training regime"

# ╔═╡ 926d35f1-3455-4e73-bd48-af7985e30677
function train()
	batch = 32
	neurons = 100
    nepochs = 250
    ...
    # Training
	data = loadSETdata()
	IC = Lux.zeros32(rng, neurons) |> gpu
    ...
    for epoch in 1:nepochs
		...
		func_array, y_expected = constructSetbatch(rng, batch, data)
		y_expected = y_expected |> gpu
		x = ArrayAndFunctionArray(IC, func_array) |> gpu
...
# ╔═╡ d7dbc0f2-244d-46e1-9832-9fefe6ad6ab4
begin
	batch = 5
	neurons = 10
    nepochs = 5
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "c1b96a158838b71b41594ef52eca4478dbea26bd"
...
[[deps.DiffEqGPU]]
deps = ["Adapt", "ArrayInterfaceGPUArrays", "CUDA", "CUDAKernels", "ChainRulesCore", "DiffEqBase", "Distributed", "DocStringExtensions", "ForwardDiff", "KernelAbstractions", "LinearAlgebra", "LinearSolve", "MuladdMacro", "Parameters", "Random", "RecursiveArrayTools", "SciMLBase", "SimpleDiffEq", "StaticArrays", "ZygoteRules"]
git-tree-sha1 = "dd110a50b6b2125aac0c10360287512cba16e198"
...
[[deps.Lux]]
deps = ["Adapt", "CUDA", "ChainRulesCore", "ComponentArrays", "FillArrays", "Flux", "Functors", "LinearAlgebra", "LuxCore", "LuxLib", "Markdown", "NNlib", "Optimisers", "Preferences", "Random", "Requires", "Setfield", "SnoopPrecompile", "SparseArrays", "Statistics", "Zygote"]
git-tree-sha1 = "d24af4b412f452c31072d465835095ca8e9aed3c"
...
git-tree-sha1 = "a083f968aa6538ce688cb99a498e1dea1a719d85"
...
git-tree-sha1 = "58369b3d052f61b6607fbaef76b745929be77712"
