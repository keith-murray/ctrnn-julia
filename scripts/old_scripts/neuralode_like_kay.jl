...
# ╔═╡ 45c68c18-68c4-4f97-bd5a-2e7f8ae04647
...
# ╔═╡ f438980a-6d4a-49cb-85ec-d0e09d984ae7
...
# ╔═╡ 2a594883-5dd1-44cb-b18e-beedb951ace5
begin
...
# ╔═╡ 0c1d808b-3826-4041-8ac6-4b25abf4f6a8
...
# ╔═╡ de4a50ee-ddbb-425d-abcc-4fba5ecce62a
struct FunctionArray{F} <: AbstractArray{F, 1}
	data::Array{F, 1}
end

# ╔═╡ c8194802-2ecb-4fce-9345-fcb1692df3ed
function constructSetbatch(rng::AbstractRNG, batch::Int64, data::Tuple{Vector{Int64}, Vector{Int64}})
	...
	return func_array, constructOutput(rng, y_expected)
end

# ╔═╡ 0901e15f-e0a1-459b-b6ac-8abd24c722c8
struct ArrayAndFunctionArray{A <: AbstractArray, B <: FunctionArray}
	array::A
	funcs::B
end

# ╔═╡ da37cfa5-911f-442c-9dcf-7ca5c5af185a
begin
...
# ╔═╡ 05c3d627-5fd6-422c-b6c2-bbc8f8cda6c5
md"# NeuralODE training like Kay et al."

# ╔═╡ 308d3278-9a3e-46ff-93ce-d0fa4d1e0cab
md"""
The purpose of this notebook is to batch train a NeuralODE with a task setup like the 'Neural dynamics and geometry for transitive inference' paper by Kay et al.
This means modifing the `Pilot NeuralODE training` notebook to include:
* Weight regularization
* Neural activation regularization
* SET properties represented as 100 dim random vectors
* fixation signal
* loss function over entire output
"""

# ╔═╡ 840781df-e1b7-4dcb-919c-16d970522fcd
md"## Setup"

# ╔═╡ 71ce9a34-5137-4427-95bc-ad31933820f8
md"## Load SET data"

# ╔═╡ 104920d2-be74-4423-860e-f0da06515e81
md"## Define NeuralODE layer"

# ╔═╡ 7e7275a4-ad5c-4fc8-895b-1e0bcd359367
md"## Define input types"

# ╔═╡ a88faa6a-0c30-49f2-8bb5-e93a18a6d3ff
md"## Define utility functions"

# ╔═╡ 7cbc7df1-f59e-46ce-aa91-a54cfd95ba8d
...
meansquarederrorTEST(y_pred, y) =  mean(abs2, y_pred[1,end,:] .- y[1,end,:])
...
function train(batch, neurons, nepochs, lr, seed)
...
md"""
Nice. The model learns on at least a fraction of the dataset. This is promising. However, it wasn't not easy or straight forward. Here are some lessons learned:
* Always seed every random operation in your code. Successes and failures should always be reproducible.
* Training a model should always begin with one data point and a low learning rate. It's better to take a long time to get the model to learn one data point than to waste time getting the model to learn many data points very quickly.
* Make multiple iterations of a notebook with well defined goals. This allows previous successes to be built off of without comprimising them. Do keep track of the order of the notebooks.
* Don't take the hyperparameters of others' work too seriously. If anything, use them as a starting point. However, you should always use common sense and your best judgement.
"""
...
[[deps.Lux]]
deps = ["Adapt", "CUDA", "ChainRulesCore", "ComponentArrays", "FillArrays", "Flux", "Functors", "LinearAlgebra", "LuxCore", "LuxLib", "Markdown", "NNlib", "Optimisers", "Random", "Requires", "ReverseDiff", "Setfield", "SparseArrays", "Statistics", "Tracker", "TruncatedStacktraces", "Zygote", "cuDNN"]
git-tree-sha1 = "adababc1531dcdb330640ab67785616d9cf31af2"
